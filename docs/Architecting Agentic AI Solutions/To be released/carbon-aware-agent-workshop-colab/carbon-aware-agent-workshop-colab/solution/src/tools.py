...
def recommend_best_from_text_tool(text_time: str) -> dict:
    """Recommend best (region, start time) from a natural-language time string (e.g. 'tomorrow 11:00').

    This is a safer wrapper around parse_time + recommend_best that prevents the LLM from inventing ISO timestamps.
    """
    start_iso = parse_time_tool(text_time)
    return recommend_best_tool(start_iso)
